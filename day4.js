...
const input = raw.map(str =>
    Object.fromEntries(str.split(" ").map(chunk => chunk.split(":")))
);
...
const valid = input.filter(doc => {
    ...
    if (keys.length >= 8) {
        return true;
    } else if (keys.length === 7 && keys.every(k => k != "cid")) {
        return true;
    }
    return false;
});

const superValid = valid.filter(doc => {
    const { byr, iyr, eyr, hgt, hcl, ecl, pid } = doc;
    const [byrI, iyrI, eyrI] = [parseInt(byr), parseInt(iyr), parseInt(eyr)];
    if (byr.length !== 4 || isNaN(byr) || byrI < 1920 || byrI > 2002) {
        return false;
    } else if (iyr.length !== 4 || isNaN(iyr) || iyrI < 2010 || iyrI > 2020) {
        return false;
    } else if (eyr.length !== 4 || isNaN(eyr) || eyrI < 2020 || eyrI > 2030) {
        return false;
    }

    const [hNum, hUnit] = [
        parseInt(hgt.slice(0, hgt.length - 2)),
        hgt.slice(hgt.length - 2),
    ];
    if (hUnit !== "cm" && hUnit !== "in") return false;
    if (hUnit === "cm" && (hNum < 150 || hNum > 193)) return false;
    if (hUnit === "in" && (hNum < 59 || hNum > 76)) return false;

    if (hcl[0] !== "#") return false;
    const hrHex = hcl.slice(1).split("");
    if (
        hrHex.length !== 6 ||
        hrHex.some(ch => !"abcdef0123456789".includes(ch.toLowerCase()))
    )
        return false;
    ...
    if (pid.length !== 9 || pid.split("").some(n => isNaN(n))) return false;

    return true;
});

console.log(superValid.length);
