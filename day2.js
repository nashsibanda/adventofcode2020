...
for (let i = 0; i < input.length; i++) {
    ...
    const test = chunks[0].split(" ");
    const amounts = test[0].split("-");
    const password = chunks[1].trim();
    const policy = {
        min: parseInt(amounts[0] - 1),
        max: parseInt(amounts[1] - 1),
        char: test[1],
    };

    let passing = false;
    if (
        password[policy["min"]] === policy["char"] &&
        password[policy["max"]] !== policy["char"]
    ) {
        passing = true;
    } else if (
        password[policy["min"]] !== policy["char"] &&
    ...
    console.log(passing, chunks);
    // if (passing) validPasswords += 1;
}
